...
module FinancialDocuments
  class StructuredSpreadsheetExtractor
    ...
    def call
      ...
      return failure("No structured Household CFO rows found") if items.empty?
      ...
      success(
        ...
        period_start_on: nil,
        period_end_on: nil,
        summary: "Mia found #{items.length} structured budget value#{'s' unless items.length == 1} for review.",
        confidence: "high",
        warnings: [],
        items: items
      )
